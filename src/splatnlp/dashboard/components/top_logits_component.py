...
top_logits_component = html.Div(id="top-logits-content", children=[
    html.H4("Top Output Logits Influenced by SAE Feature", className="mb-3"),
    dcc.Loading(
...
def update_top_logits_graph(selected_feature_id):
    ...
    if selected_feature_id is None:
        return {"data": [], "layout": {"title": "Select an SAE feature to see logit influences"}}, ""
    if DASHBOARD_CONTEXT is None:
        return {"data": [], "layout": {"title": "Dashboard context not loaded"}}, "Error: DASHBOARD_CONTEXT is None. Ensure data is loaded."

    sae_model = DASHBOARD_CONTEXT.sae_model
    primary_model = DASHBOARD_CONTEXT.primary_model
    inv_vocab = DASHBOARD_CONTEXT.inv_vocab # Expected: {int_index: token_name_str}

    if not hasattr(sae_model, 'decoder') or not hasattr(sae_model.decoder, 'weight'):
        return {"data": [], "layout": {"title": "SAE decoder weights not found"}}, "Error: SAE decoder or its weights not found."
    if not hasattr(sae_model, 'hidden_dim'):
        return {"data": [], "layout": {"title": "SAE hidden_dim not found"}}, "Error: SAE hidden_dim attribute not found on sae_model."
    
    if not hasattr(primary_model, 'output_layer') or not hasattr(primary_model.output_layer, 'weight'):
        return {"data": [], "layout": {"title": "Primary model output layer weights not found"}}, "Error: Primary model output_layer or its weights not found."
    ...
    try:
        sae_feature_effect_on_primary_activation = sae_model.decoder.weight.data[:, selected_feature_id].detach().cpu()
    except IndexError:
        ...
        return {"data": [], "layout": {"title": "SAE Weight Access Error"}}, err_msg
    ...
    primary_output_layer_weights = primary_model.output_layer.weight.data.detach().cpu()

    if sae_feature_effect_on_primary_activation.shape[0] != primary_output_layer_weights.shape[1]:
        err_msg = (f"Dimension mismatch: SAE feature effect vector size ({sae_feature_effect_on_primary_activation.shape[0]}) "
                   f"does not match primary model output layer input dimension ({primary_output_layer_weights.shape[1]}).")
        return {"data": [], "layout": {"title": "Dimension Mismatch"}}, "Error: " + err_msg

    # (vocab_size, hidden_dim_primary) @ (hidden_dim_primary) -> (vocab_size)
    effect_on_logits = torch.matmul(primary_output_layer_weights, sae_feature_effect_on_primary_activation)
    ...
    combined_indices = sorted(list(combined_indices_set), key=lambda x: effect_on_logits_np[x], reverse=True)
    ...
    for idx in combined_indices:
        ...
        if token_name == f"Token_ID_{token_idx_int}": # Fallback if int key didn't work
             token_name = inv_vocab.get(str(token_idx_int), f"Token_ID_{token_idx_int}")

        results.append({
            "token": token_name, 
            "effect": effect_on_logits_np[idx], 
            "type": "Positive" if effect_on_logits_np[idx] >= 0 else "Negative"
        })

    if not results:
        return {"data": [], "layout": {"title": f"No significant logit effects found for Feature {selected_feature_id}"}}, ""

    fig = px.bar(results, x="token", y="effect", color="type",
