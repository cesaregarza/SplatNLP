...
def format_example_with_tooltips(
    ...
    weapon_name = inv_weapon_vocab.get(str(weapon_id_token), inv_weapon_vocab.get(weapon_id_token, f"WeaponID_{weapon_id_token}"))
    
    top_pred_str = "N/A" # Default
    if model_logits is not None and isinstance(model_logits, np.ndarray) and model_logits.size > 0:
        ...
        if actual_k > 0:
            ...
            token_name = inv_vocab.get(str(top_idx), inv_vocab.get(top_idx, f"Token_ID_{top_idx}"))
    ...
    token_projections = [] # For storing text like "TokenName: 0.123" or error messages
    if token_activations_accessor and sae_feature_direction is not None:
        try:
            ...
            for k, token_id in enumerate(ability_token_ids):
                ...
                if token_name_for_proj in ("<PAD>", "<NULL>"): continue # Skip PAD/NULL

                if k < token_hidden_states_for_example.shape[0]:
                    token_primary_act_vector = token_hidden_states_for_example[k, :]
                    projection = np.dot(token_primary_act_vector, sae_feature_direction)
                    token_projections.append(f"{token_name_for_proj}: {projection:.3f}")
                else:
                    token_projections.append(f"{token_name_for_proj}: (act_idx_err)")
        except KeyError:
            token_projections = [f"(Token acts for ex {example_idx} missing)"]
        except Exception: # Broad exception for safety during HDF5 access / np.dot
            token_projections = [f"(Error processing token acts for ex {example_idx})"]
    ...
    if not ability_names or all(name in ("<PAD>", "<NULL>") for name in ability_names):
        input_abilities_spans.append(html.Span("N/A"))
    else:
        current_token_idx_for_projection = 0
        for i, token_name in enumerate(ability_names):
            if token_name in ("<PAD>", "<NULL>"): continue
            
            span_id = f"interval-{interval_num}-ex-{example_in_interval_num}-token-{i}"
            input_abilities_spans.append(html.Span(token_name, id=span_id, style={"margin-right": "5px", "text-decoration": "underline dotted"}))
            ...
            if current_token_idx_for_projection < len(token_projections):
                ...
                elif proj_info.startswith("("): # It's an error/placeholder message for the whole example
                    tooltip_text = proj_info 
            ...
            tooltip_components.append(dbc.Tooltip(tooltip_text, target=span_id, placement="top"))
            current_token_idx_for_projection +=1
            if i < len(ability_names) -1 and ability_names[i+1] not in ("<PAD>", "<NULL>"):
    ...
    ], style={"border": "1px solid #eee", "padding": "5px", "margin-bottom": "5px"}), tooltip_components
...
def update_intervals_grid(selected_feature_id):
    ...
    if sae_model and hasattr(sae_model, 'decoder') and hasattr(sae_model.decoder, 'weight'):
        ...
        if 0 <= selected_feature_id < sae_decoder_weights.shape[1]:
            ...
            current_run_error_messages.append(f"Warning: Selected feature ID {selected_feature_id} invalid for SAE decoder. Projections disabled.")
            sae_feature_direction = None
    elif sae_model is None:
        current_run_error_messages.append("Warning: SAE model not found. Projections disabled.")

    if not token_activations_accessor:
        current_run_error_messages.append("Warning: Token activations HDF5 not loaded. Projections disabled.")
    
    if not (0 <= selected_feature_id < all_sae_acts.shape[1]):
        current_run_error_messages.append(f"Error: Feature ID {selected_feature_id} out of range for all_sae_acts.")
    ...
    max_act = max_act + 1e-6 if max_act == min_act else max_act # Avoid zero range for linspace
    ...
    for i in range(num_intervals):
        ...
        condition = (feature_activations >= lower_bound) & \
                    (feature_activations < upper_bound if i < num_intervals - 1 else feature_activations <= upper_bound)
        
        indices_in_interval = np.where(condition)[0]
        interval_section_children = [html.H6(f"Interval {i+1}: [{lower_bound:.3f}, {upper_bound:.3f}) - {len(indices_in_interval)} examples")]
        ...
        if len(indices_in_interval) > 0:
            ...
            for ex_in_interval_idx, original_example_idx in enumerate(sampled_indices):
                if original_example_idx >= len(analysis_df): continue # Should not happen
                ...
                example_div, example_tooltips = format_example_with_tooltips(
                    original_example_idx, record, inv_vocab, inv_weapon_vocab, 
                    activation_value, model_logits,
                    token_activations_accessor, sae_feature_direction,
                    i, ex_in_interval_idx # Pass interval and example numbers for unique IDs
                )
                interval_section_children.append(example_div)
                all_tooltips_for_page.extend(example_tooltips)
        else:
            interval_section_children.append(html.P("No examples in this interval."))
        
        grid_layout_children.append(html.Div(interval_section_children, className="interval-section", style={'margin-bottom': '15px'}))
    ...
    if not grid_layout_children: # Should always have interval sections, but as a fallback
         current_run_error_messages.append("Could not generate interval grid display.")
