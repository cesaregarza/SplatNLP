...
correlations_component = html.Div(id="correlations-content", children=[
    html.H4("Feature Correlations", className="mb-3"),
    dcc.Loading(
        id="loading-correlations",
        type="default",
        children=[
            html.Div(id="sae-feature-correlations-display", className="mb-3"),
            html.Hr(),
...
def calculate_top_correlated_features(all_sae_acts, selected_feature_id, top_n=5):
    ...
    if selected_feature_id >= all_sae_acts.shape[1]:
        return [], f"Selected feature ID {selected_feature_id} out of range for SAE activations (max: {all_sae_acts.shape[1]-1})."
    ...
    for i in range(all_sae_acts.shape[1]):
        ...
        if target_feature_vector.size < 2 or other_feature_vector.size < 2:
            corr = 0
        elif np.std(target_feature_vector) == 0 or np.std(other_feature_vector) == 0:
            corr = 0
        else:
            ...
            try:
                ...
                corr = 0
        correlations.append({"feature_id": i, "correlation": corr if np.isfinite(corr) else 0})
    
    sorted_correlations = sorted(correlations, key=lambda x: abs(x["correlation"]), reverse=True)
    return sorted_correlations[:top_n], None

def calculate_top_token_logit_correlations(all_sae_acts, analysis_df, selected_feature_id, inv_vocab, top_n=10):
    if not isinstance(all_sae_acts, np.ndarray):
        return [], "SAE activations are not a NumPy array."
    if selected_feature_id >= all_sae_acts.shape[1]:
        return [], f"Selected feature ID {selected_feature_id} out of range for SAE activations (max: {all_sae_acts.shape[1]-1})."
    ...
    try:
        ...
        # Check if elements are lists or Series (which can be converted to lists)
            ...
            return [], f"Unsupported type for model_logits: {type(analysis_df['model_logits'].iloc[0])}."
    ...
    if all_model_logits.ndim != 2: # Expected shape (num_examples, vocab_size)
        return [], f"Expected model_logits to be 2D, but got shape {all_model_logits.shape} after conversion."
    if all_model_logits.shape[0] != target_feature_activations.shape[0]:
        return [], f"Mismatch in number of examples: SAE activations ({target_feature_activations.shape[0]}) vs model_logits ({all_model_logits.shape[0]})."
    ...
    for i in range(vocab_size):
        ...
        if target_feature_activations.size < 2 or logit_values_for_token_i.size < 2:
            corr = 0
        elif np.std(target_feature_activations) == 0 or np.std(logit_values_for_token_i) == 0:
            corr = 0
        else:
            try:
                corr, _ = pearsonr(target_feature_activations, logit_values_for_token_i)
            except ValueError:
                corr = 0
        
        # Ensure token_id 'i' is correctly used for inv_vocab
        token_name = inv_vocab.get(str(i), inv_vocab.get(int(i), f"TokenID_{i}"))
        correlations.append({"token_name": token_name, "token_id": i, "correlation": corr if np.isfinite(corr) else 0})
...
def update_correlations_display(selected_feature_id):
    ...
    try:
        ...
        if err:
            ...
            sae_corr_display_children.append(html.H5("Top Correlated SAE Features (Pearson):", className="mb-2"))
            if top_sae_features:
                for item in top_sae_features:
                    sae_corr_display_children.append(html.P(f"Feature {item['feature_id']}: {item['correlation']:.3f}", className="ms-3"))
            else:
                sae_corr_display_children.append(html.P("No significant SAE feature correlations found or calculable."))
        
        # 2. Token-Logit Correlations
        # Pass the converted analysis_df to the helper
        top_token_corrs, err = calculate_top_token_logit_correlations(all_sae_acts, analysis_df, selected_feature_id, inv_vocab)
        if err:
            error_message_parts.append(f"Token-Logit Corr Error: {err}")
        else:
            token_corr_display_children.append(html.H5("Top Correlated Output Tokens (Logit Value vs SAE Feature Activation):", className="mb-2"))
            if top_token_corrs:
                for item in top_token_corrs:
                    token_corr_display_children.append(html.P(f"Token '{item['token_name']}' (ID: {item['token_id']}): {item['correlation']:.3f}", className="ms-3"))
            else:
                token_corr_display_children.append(html.P("No significant token-logit correlations found or calculable."))
            
        final_error_message = " | ".join(error_message_parts) if error_message_parts else ""
        return sae_corr_display_children, token_corr_display_children, final_error_message

    except Exception as e:
        return [], [], f"An unexpected error occurred in correlations component: {str(e)}"
