...
@callback(
    ...
    Input("page-load-trigger", "data"), # Dummy input to trigger on page load after context is set
    State("feature-dropdown", "value"), # Current value to preserve if already set by URL
    State("url", "search")
)
def populate_feature_options(page_load_data, current_value, search_query):
    ...
    if DASHBOARD_CONTEXT is not None and hasattr(DASHBOARD_CONTEXT, 'sae_model') and \
       DASHBOARD_CONTEXT.sae_model is not None and hasattr(DASHBOARD_CONTEXT.sae_model, 'hidden_dim'):
        ...
        if num_features > 0:
            ...
            options = [{"label": "No features available", "value": -1, "disabled": True}]
            default_value = -1
    else:
        options = [{"label": "Context/Model not loaded", "value": -1, "disabled": True}]
        default_value = -1
    ...
    if determined_value == default_value and current_value is not None:
        if any(opt['value'] == current_value for opt in options if not opt.get('disabled')):
    ...
    if not any(opt['value'] == final_value for opt in options if not opt.get('disabled')):
        if options and not options[0].get('disabled'):
            final_value = options[0]['value'] # Fallback to the first available option
        elif options and options[0].get('disabled'):
            final_value = options[0]['value'] # Fallback to the disabled value like -1
...
def update_url_on_dropdown_change(selected_feature_id):
    if selected_feature_id is not None and selected_feature_id != -1: # -1 used for disabled/no feature states
