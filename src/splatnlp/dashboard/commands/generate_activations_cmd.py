...
def _generate_dashboard_activations_no_chunk(args: argparse.Namespace):
    ...
    all_sae_hidden_activations = (
        ...
        if all_records
        else np.empty((0, args.hidden_dim * int(args.sae_expansion_factor)))
...
def _generate_dashboard_activations_chunked(args: argparse.Namespace):
    ...
    chunk_storage_dir = getattr(
        args, "chunk_storage_dir", "/mnt/e/activations_default_cmd_chunked"
    )
...
def generate_activations_command(args: argparse.Namespace):
    ...
    if effective_fraction > 0 and effective_fraction > effective_chunk_size:
        ...
        if (
            ...
            args.chunk_storage_dir = (
                "/mnt/e/activations_cmd_default_chunked_main"
            )
            logger.info(
        ...
        if not output_path.exists() or args.force:
            ...
            chunk_storage_dir = getattr(
                args,
                "chunk_storage_dir",
                "/mnt/e/activations_default_empty_cmd_main",
            )
