...
def _generate_dashboard_activations_no_chunk(args: argparse.Namespace):
    ...
    if any(
        ...
        checkpoint = torch.load(
            args.primary_model_checkpoint, map_location="cpu"
        )

        if args.embedding_dim is None:
            embed_key = [
                k for k in checkpoint.keys() if "ability_embedding.weight" in k
            ][0]
            _, args.embedding_dim = checkpoint[embed_key].shape
            logger.info(f"  Detected embedding_dim: {args.embedding_dim}")

        if args.hidden_dim is None:
            ff_key = [
                k
                for k in checkpoint.keys()
                if "feedforward.0.weight" in k and "transformer_layers.0" in k
            ][0]
            _, args.hidden_dim = checkpoint[ff_key].shape
            logger.info(f"  Detected hidden_dim: {args.hidden_dim}")

        if args.num_layers is None:
            layer_keys = set()
            for k in checkpoint.keys():
                if "transformer_layers." in k:
                    layer_idx = k.split("transformer_layers.")[1].split(".")[0]
                    layer_keys.add(int(layer_idx))
            args.num_layers = len(layer_keys)
            logger.info(f"  Detected num_layers: {args.num_layers}")

        if args.num_heads is None:
            # Estimate based on hidden_dim
            if args.hidden_dim == 512:
                ...
                args.num_heads = 8
            logger.info(f"  Estimated num_heads: {args.num_heads}")

        if args.num_inducing_points is None:
            ind_key = [k for k in checkpoint.keys() if "inducing_points" in k][
                0
            ]
            args.num_inducing_points = checkpoint[ind_key].shape[0]
            logger.info(
                f"  Detected num_inducing_points: {args.num_inducing_points}"
            )
    ...
    # Load SAE checkpoint to auto-detect parameters
    logger.info("Loading SAE model...")
    sae_checkpoint = torch.load(
        args.sae_model_checkpoint, map_location=torch.device("cpu")
    )

    # Auto-detect SAE parameters from checkpoint
    sae_input_dim = args.hidden_dim  # Default to hidden_dim
    expansion_factor = 4  # Default value

    # Try to detect from encoder weight shape
    if "encoder.weight" in sae_checkpoint:
        encoder_weight = sae_checkpoint["encoder.weight"]
        sae_hidden_dim, sae_input_dim = encoder_weight.shape
        expansion_factor = sae_hidden_dim / sae_input_dim
        logger.info(
            f"Auto-detected SAE parameters: input_dim={sae_input_dim}, hidden_dim={sae_hidden_dim}, expansion_factor={expansion_factor}"
        )
    else:
        logger.warning(
            "Could not auto-detect SAE parameters from checkpoint, using defaults"
        )

    sae_model = SparseAutoencoder(
        input_dim=int(sae_input_dim),
    ...
    # Get num_instances_per_set from args, default to 1 for Full model
    num_instances = getattr(args, "num_instances_per_set", 1)
    ...
    dataloader, _, _ = generate_dataloaders(
        ...
        vocab_size=len(vocab),
        pad_token_id=pad_token_id,
        num_instances_per_set=num_instances,
        skew_factor=1.2,
    ...
    all_sae_hidden_activations = (
        np.stack(analysis_df["sae_hidden"].to_list(), axis=0)
        if all_records
        else np.empty((0, int(sae_input_dim * expansion_factor)))
    )  # handle empty all_records
    ...
    if (
        all_sae_hidden_activations.ndim == 2
    ):  # Check ndim before accessing shape[1]
        logger.info(
...
def collect_activations_chunk(
    ...
    if hidden_activations:  # only stack if list is not empty
        ...
        feature_dim = (
            sae_model.b_dec.shape[0]
            if hasattr(sae_model, "b_dec") and sae_model.b_dec is not None
            else (
                sae_model.W_enc.shape[1]
                if hasattr(sae_model, "W_enc") and sae_model.W_enc is not None
                else -1
            )
        )
        if feature_dim != -1:
            ...
            hidden_activations_array = np.array(
                hidden_activations
            )  # Might result in object array if shapes are inconsistent

    logger.info(
        f"Collected activations for {len(records)} records, hidden_array_shape: {hidden_activations_array.shape}"
    )
...
def save_chunked_activations(
        ...
        for chunk_file_path_str in chunk_files:
            try:
                with np.load(chunk_file_path_str) as data:
                    if (
                        "hidden_activations" in data
                        and data["hidden_activations"].ndim == 2
                        and data["hidden_activations"].shape[0] > 0
                    ):
                        feature_dim = data["hidden_activations"].shape[1]
                        break
            except Exception as e:
                logger.warning(
                    f"Could not read shape from chunk {chunk_file_path_str}: {e}"
                )

    if (
        feature_dim == -1
        and len(null_hidden_activations) > 0
        and null_hidden_activations.ndim == 2
        and null_hidden_activations.shape[0] > 0
    ):
    ...
    if feature_dim == -1:  # If still undetermined
        ...
        logger.info(
            f"Feature dimension undetermined from data, using sae_output_dim: {feature_dim}"
        )

    if (
        total_records == 0 and feature_dim == 0
    ):  # No records and no features, likely an issue
        logger.warning(
            "No records and no features. Saving metadata only for empty cache."
        )
        # Fall through to metadata saving, HDF5 will be empty if created

    logger.info(
        f"Target HDF5 file: {h5_path} with shape ({total_records}, {feature_dim})"
    )
    ...
    with h5py.File(h5_path, "w") as h5f:
            ...
            try:
                with np.load(chunk_file) as data:
                    ...
                    if (
                        ...
                        and chunk_data.ndim == 2
                        and chunk_data.shape[0] > 0
                    ):
                        if (
                            chunk_data.shape[1] == feature_dim
                        ):  # Check consistency
                            chunk_size = chunk_data.shape[0]
                            if dset is not None:  # Ensure dset exists
                                dset[current_idx : current_idx + chunk_size] = (
                                    chunk_data
                                )
                            current_idx += chunk_size
                        else:
                            logger.warning(
                                f"Inconsistent feature dimension in chunk {chunk_file}. Expected {feature_dim}, got {chunk_data.shape[1]}. Skipping."
                            )
                    elif chunk_data is not None and chunk_data.shape[0] == 0:
                        logger.info(
                            f"Skipping empty data in chunk: {chunk_file}"
                        )
        ...
        if (
            len(null_hidden_activations) > 0
            and null_hidden_activations.ndim == 2
            and null_hidden_activations.shape[0] > 0
        ):
            if (
                null_hidden_activations.shape[1] == feature_dim
            ):  # Check consistency
                if dset is not None:  # Ensure dset exists
                    dset[
                        current_idx : current_idx + len(null_hidden_activations)
                    ] = null_hidden_activations
            else:
                logger.warning(
                    f"Inconsistent feature dimension in null_hidden_activations. Expected {feature_dim}, got {null_hidden_activations.shape[1]}. Skipping."
                )
    ...
    logger.info(
        f"Cache saved: {output_path} (pointer), {metadata_path} (metadata), {h5_path if (total_records > 0 and feature_dim > 0) else '(no data H5)'} (data)"
    )
...
def _generate_dashboard_activations_chunked(args: argparse.Namespace):
    ...
    primary_model.load_state_dict(
        torch.load(args.primary_model_checkpoint, map_location="cpu")
    )
    primary_model.to(device).eval()
    ...
    sae_model.load_state_dict(sae_checkpoint)
    sae_model.to(device).eval()
    sae_output_dim = int(sae_input_dim * expansion_factor)

    output_path = Path(args.output_path)
    if output_path.exists() and not args.force:
        logger.info(
            f"Cache already exists at {output_path}. Use --force to regenerate"
        )
        return
    ...
    if args.fraction == 0.0:
        logger.warning(
            "Fraction is 0, no data will be sampled for regular activations."
        )
        sampled_df = pd.DataFrame(
            columns=df_full.columns if not df_full.empty else None
        )
    else:
        sampled_df = (
            df_full.sample(
    ...
    logger.info(
        f"Sampled {len(sampled_df)} rows ({args.fraction:.1%} of {len(df_full)})"
    )

    chunk_storage_dir = getattr(
        args, "chunk_storage_dir", "/mnt/e/activations_ultra/chunks"
    )
    ...
    args.chunk_size = max(
        0.0001,
        min(args.chunk_size, args.fraction if args.fraction > 0 else 1.0),
    )
    ...
    if len(sampled_df) > 0:
        rows_per_chunk = (
            max(1, int(len(df_full) * args.chunk_size))
            if len(df_full) > 0
            else 1
        )  # handle empty df_full
        num_chunks = (
            int(np.ceil(len(sampled_df) / rows_per_chunk))
            if rows_per_chunk > 0
            else 0
        )
        ...
        for chunk_idx in range(num_chunks):
            ...
            if chunk_file.exists() and not args.force:
                logger.info(
                    f"Chunk {chunk_file} exists, attempting to load metadata..."
                )
                try:
                    with np.load(chunk_file) as data:
                        num_records = int(data["num_records"])
                    all_records_metadata.extend(
                        [{"is_null_token": False} for _ in range(num_records)]
                    )
                    logger.info(
                        f"Loaded metadata for {num_records} records from existing chunk."
                    )
                    continue
                except Exception as e:
                    logger.warning(
                        f"Could not load existing chunk {chunk_file}: {e}. Will attempt to regenerate."
                    )

            chunk_df_current = sampled_df.iloc[chunk_start:chunk_end].copy()
            if len(chunk_df_current) == 0:
                logger.info(
                    f"Skipping empty chunk {chunk_idx + 1}/{num_chunks}"
                )
                np.savez_compressed(
                    chunk_file,
                    hidden_activations=np.empty((0, sae_output_dim)),
                    num_records=0,
                )
                continue

            chunk_loader, _, _ = generate_dataloaders(
                train_set=chunk_df_current,
                validation_set=chunk_df_current,
                test_set=chunk_df_current,
                vocab_size=len(vocab),
                pad_token_id=pad_token_id,
                num_instances_per_set=num_instances,
                skew_factor=1.2,
            ...
            if (
                chunk_activations.ndim == 2
                and chunk_activations.shape[0] == 0
                and chunk_activations.shape[1] == 0
            ):
                np.savez_compressed(
                    chunk_file,
                    hidden_activations=np.empty((0, sae_output_dim)),
                ...
                np.savez_compressed(
                    chunk_file,
                    hidden_activations=chunk_activations,
                    num_records=len(chunk_records),
                )
        ...
        logger.info(
            "No regular data to process based on fraction and sampling."
        )
    ...
    if null_records_full:
        null_hidden_activations = np.stack(
            [r["sae_hidden"] for r in null_records_full], axis=0
        )
        null_metadata_records = [
            {
                ...
                for k, v in r.items()
                if k
                not in ["sae_input", "sae_hidden", "sae_recon", "model_logits"]
            }
            for r in null_records_full
        ]
    else:
...
def generate_activations_command(args: argparse.Namespace):
    ...
    if effective_chunk_size <= 0:
        logger.warning(
            "args.chunk_size not set or invalid, defaulting to 0.01."
        )
    ...
    # Load SAE to get output dimension
    sae_checkpoint = torch.load(
        args.sae_model_checkpoint, map_location=torch.device("cpu")
    )
    if "encoder.weight" in sae_checkpoint:
        encoder_weight = sae_checkpoint["encoder.weight"]
        sae_hidden_dim, sae_input_dim = encoder_weight.shape
        sae_output_dim = sae_hidden_dim
    else:
        # Fallback to calculation from args
        sae_output_dim = args.hidden_dim * int(
            getattr(args, "sae_expansion_factor", 4)
        )

    if effective_fraction > 0 and effective_fraction > effective_chunk_size:
        logger.info(
            f"Using chunked processing (fraction {effective_fraction} > chunk_size {effective_chunk_size})"
        )
        if (
            not hasattr(args, "chunk_storage_dir")
            or args.chunk_storage_dir is None
        ):
            args.chunk_storage_dir = "/mnt/e/activations_ultra/chunks"
            logger.info(
                f"args.chunk_storage_dir not provided, defaulting to {args.chunk_storage_dir}"
            )
        _generate_dashboard_activations_chunked(args)
    elif effective_fraction > 0:
        logger.info(
            f"Using non-chunked processing (fraction {effective_fraction})"
        )
        ...
        if not output_path.exists() or args.force:
            logger.info(
                "Creating empty cache structure because fraction is 0 and (cache missing or --force)."
            )
            vocab = load_vocab(args.vocab_path)
            weapon_vocab = load_vocab(args.weapon_vocab_path)

            empty_analysis_df = pd.DataFrame(
                columns=[
            ...
            if hasattr(args, "chunk_size"):
                metadata_payload["chunk_size"] = effective_chunk_size

            chunk_storage_dir = getattr(
                args,
                "chunk_storage_dir",
                "/mnt/e/activations_ultra/chunks_empty",
            )
            ...
            logger.info(
                f"Cache already exists at {output_path}, fraction is 0, and --force not used. No action taken."
            )
