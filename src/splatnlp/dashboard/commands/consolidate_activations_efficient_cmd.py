...
from typing import Dict, List, Optional # Ensure all necessary types are imported
import logging
...
def consolidate_to_hdf5_streaming(
    ...
    """
    logger.info(f"Starting consolidation from cache directory: {cache_dir} to output: {output_file}")
    ...
    try:
        with np.load(chunk_files[0]) as first_chunk_data:
            if 'hidden_activations' not in first_chunk_data:
                logger.error(f"Key 'hidden_activations' not found in first chunk: {chunk_files[0]}")
                raise KeyError(f"Key 'hidden_activations' not found in first chunk: {chunk_files[0]}")
            # Ensure it's a 2D array before accessing shape[1]
            if first_chunk_data['hidden_activations'].ndim != 2:
                logger.error(f"'hidden_activations' in {chunk_files[0]} is not 2D (shape: {first_chunk_data['hidden_activations'].shape})")
                raise ValueError(f"'hidden_activations' in {chunk_files[0]} is not 2D")
            n_features = first_chunk_data['hidden_activations'].shape[1]
    except Exception as e:
        logger.error(f"Could not read first chunk {chunk_files[0]} to determine dimensions: {e}")
        raise
    ...
    logger.info("Scanning chunks to count total examples and gather metadata...")
    for i, chunk_file_path in enumerate(tqdm(chunk_files, desc="Scanning chunks")):
        try:
            with np.load(chunk_file_path) as chunk_data_scan:
                num_records_in_chunk = int(chunk_data_scan.get('num_records', 0)) # Default to 0 if not found
                hidden_acts_shape = chunk_data_scan.get('hidden_activations', np.array([])).shape

                if num_records_in_chunk == 0 and hidden_acts_shape[0] != 0 :
                    logger.warning(f"Chunk {chunk_file_path} has num_records=0 but hidden_activations shape {hidden_acts_shape}. Using shape[0].")
                    num_records_in_chunk = hidden_acts_shape[0]
                elif num_records_in_chunk != 0 and hidden_acts_shape[0] == 0 and num_records_in_chunk > 0:
                     logger.warning(f"Chunk {chunk_file_path} has num_records={num_records_in_chunk} but hidden_activations is empty. Assuming num_records is correct.")
                elif num_records_in_chunk != hidden_acts_shape[0] and hidden_acts_shape[0] != 0: # if hidden_acts_shape[0] is 0, num_records might be right
                    logger.warning(f"Mismatch in chunk {chunk_file_path}: num_records={num_records_in_chunk}, activations shape={hidden_acts_shape}. Using num_records.")
                ...
                for record_idx_in_chunk in range(num_records_in_chunk):
                    ...
                    per_chunk_metadata_list.append({
                        ...
                        'global_example_id_in_consolidation': total_examples - num_records_in_chunk + record_idx_in_chunk # Tentative global ID
                    })
        except Exception as e:
            logger.error(f"Error scanning chunk {chunk_file_path}: {e}. Skipping this chunk for totals.")
            continue # Skip corrupted chunk

    if total_examples == 0:
        logger.warning("Total examples from chunks is 0. Output HDF5 will be empty.")
    if n_features == 0: # Should ideally not happen if first chunk was valid
        logger.warning("Number of features is 0. Output HDF5 will have 0 columns for activations.")


    logger.info(f"Consolidating {total_examples} total examples, {n_features} features per example.")
    
    output_file.parent.mkdir(parents=True, exist_ok=True)
    with h5py.File(output_file, 'w') as hf:
        acts_dataset = hf.create_dataset(
            'activations',
            shape=(total_examples, n_features), # Ensure shape is correctly determined
            dtype='float32',
            # Chunks for HDF5 - good defaults
            chunks=(min(1024, total_examples if total_examples > 0 else 1), n_features if n_features > 0 else 1),
            compression='gzip',
        ...
        for i, chunk_file_path in enumerate(tqdm(chunk_files, desc="Writing chunks to HDF5")):
            try:
                with np.load(chunk_file_path) as chunk_data_load:
                    chunk_acts_data = chunk_data_load.get('hidden_activations')
                    num_records_meta = int(chunk_data_load.get('num_records', 0))

                    if chunk_acts_data is None:
                        logger.warning(f"Chunk {chunk_file_path} missing 'hidden_activations'. Skipping.")
                        continue
                    if chunk_acts_data.ndim != 2 or chunk_acts_data.shape[1] != n_features:
                        logger.warning(f"Chunk {chunk_file_path} has unexpected activation shape {chunk_acts_data.shape} (expected N x {n_features}). Skipping.")
                        continue
                    
                    n_examples_in_current_chunk = chunk_acts_data.shape[0]

                    if n_examples_in_current_chunk == 0 and num_records_meta > 0:
                        ...
                        logger.warning(f"Chunk {chunk_file_path}: 'hidden_activations' is empty but num_records={num_records_meta}. Writing no activation data for this chunk's records.")
                    ...
                    if n_examples_in_current_chunk > 0 :
                        if current_example_write_idx + n_examples_in_current_chunk > total_examples:
                            logger.error(f"Exceeding total_examples ({total_examples}) while writing chunk {chunk_file_path}. Current write index {current_example_write_idx}, chunk size {n_examples_in_current_chunk}. Stopping.")
                            break # Avoid writing out of bounds

                        acts_dataset[current_example_write_idx : current_example_write_idx + n_examples_in_current_chunk] = chunk_acts_data
                    
                    # Construct metadata for records in this chunk
                    for record_idx_in_chunk_iter in range(n_examples_in_current_chunk): # Iterate based on actual data written
                        consolidated_metadata_records.append({
                            ...
                            'consolidated_global_example_id': current_example_write_idx + record_idx_in_chunk_iter
                        })
                    current_example_write_idx += n_examples_in_current_chunk

            except Exception as e:
                logger.error(f"Error processing or writing chunk {chunk_file_path}: {e}. Skipping.")
        ...
        if current_example_write_idx != total_examples:
            logger.warning(f"Final write index {current_example_write_idx} does not match total expected examples {total_examples}. HDF5 file might be smaller or there were errors.")
    ...
    metadata_output_file = output_file.with_suffix('.metadata.pkl') # Or .csv, .parquet
    try:
        metadata_df.to_pickle(metadata_output_file) # Pandas pickle is generally fine
        logger.info(f"Consolidated metadata saved to {metadata_output_file} ({len(metadata_df)} records)")
    except Exception as e:
        logger.error(f"Failed to save consolidated metadata: {e}")

    logger.info(f"Consolidation complete. Activations: {output_file}, Metadata: {metadata_output_file}")
...
def create_dashboard_cache(
    ...
    try:
        ...
        logger.info(f"Loaded metadata for {n_total_examples_from_meta} examples from {metadata_file}")
    except Exception as e:
    ...
    try:
        with h5py.File(activations_file, 'r') as hf:
            ...
            if n_total_examples_from_meta != n_total_examples_in_h5:
                logger.warning(f"Mismatch in example count: metadata has {n_total_examples_from_meta}, HDF5 has {n_total_examples_in_h5}. Using HDF5 count for sampling.")
            ...
            n_neurons_to_sample_actual = min(n_neurons_sample, n_total_features_in_h5)
            if n_neurons_to_sample_actual == 0 :
                 logger.warning(f"No features in HDF5 file {activations_file}, cannot sample neurons.")
                 sampled_neuron_indices = np.array([], dtype=int)
            elif n_neurons_to_sample_actual < 0: # Should not happen with min
                 logger.error(f"n_neurons_sample is negative: {n_neurons_to_sample_actual}")
                 return
            else:
                 sampled_neuron_indices = np.linspace(0, n_total_features_in_h5 - 1, n_neurons_to_sample_actual, dtype=int)
            
            # Sample example indices (randomly)
            # Ensure n_examples_sample is not more than available examples
            n_examples_to_sample_actual = min(n_examples_sample, actual_n_examples_to_use)
            if n_examples_to_sample_actual == 0:
                logger.warning(f"No examples in HDF5 file {activations_file} or n_examples_sample is 0, cannot sample examples.")
                sampled_example_indices = np.array([], dtype=int)
                sample_activations_matrix = np.zeros((0, n_neurons_to_sample_actual), dtype=np.float32)
            elif n_examples_to_sample_actual < 0:
                 logger.error(f"n_examples_sample is negative: {n_examples_to_sample_actual}")
                 return
            else:
                sampled_example_indices = np.random.choice(actual_n_examples_to_use, n_examples_to_sample_actual, replace=False)
                ...
                if n_neurons_to_sample_actual > 0 and n_examples_to_sample_actual > 0:
                    sample_activations_matrix = np.zeros((n_examples_to_sample_actual, n_neurons_to_sample_actual), dtype=np.float32)
                    for i, neuron_idx_val in enumerate(tqdm(sampled_neuron_indices, desc="Sampling neuron activations")):
                        sample_activations_matrix[:, i] = acts_dataset[sampled_example_indices, neuron_idx_val]
                else: # Handle cases where no neurons or no examples are sampled
                    sample_activations_matrix = np.zeros((n_examples_to_sample_actual, n_neurons_to_sample_actual), dtype=np.float32)


    except Exception as e:
        logger.error(f"Error during HDF5 processing or sampling for dashboard cache: {e}")
        return
    
    # Save sample activation data
    dashboard_sample_file = dashboard_cache_output_dir / 'dashboard_sample_activations.npz'
    try:
        np.savez_compressed(
        ...
        logger.info(f"Dashboard sample activations saved to {dashboard_sample_file}")
    except Exception as e:
    ...
    valid_sampled_example_indices_for_meta = [idx for idx in sampled_example_indices if idx < len(metadata_df)]
    if len(valid_sampled_example_indices_for_meta) != len(sampled_example_indices):
        logger.warning("Some sampled example indices were out of bounds for the loaded metadata. Sampled metadata will be smaller.")

    sample_metadata_df = metadata_df.iloc[valid_sampled_example_indices_for_meta].copy()
    # Add a column for the original HDF5 index if it's not already the DataFrame index
    sample_metadata_df['original_hdf5_index'] = valid_sampled_example_indices_for_meta

    sample_metadata_output_file = dashboard_cache_output_dir / 'dashboard_sample_metadata.pkl'
    try:
        sample_metadata_df.to_pickle(sample_metadata_output_file)
        logger.info(f"Dashboard sample metadata saved to {sample_metadata_output_file}")
    except Exception as e:
    ...
    dashboard_index_data = {
        'n_total_neurons': n_total_features_in_h5 if 'n_total_features_in_h5' in locals() else -1,
        'n_total_examples': actual_n_examples_to_use if 'actual_n_examples_to_use' in locals() else -1,
        'n_neurons_in_sample': len(sampled_neuron_indices) if 'sampled_neuron_indices' in locals() else -1,
        'n_examples_in_sample': len(sampled_example_indices) if 'sampled_example_indices' in locals() else -1,
        'path_to_full_activations_h5': str(activations_file.resolve()),
        'path_to_full_metadata_pkl': str(metadata_file.resolve()),
        'path_to_neuron_summaries_dir': str(neuron_summary_dir.resolve()), # From extract_top_examples
        'path_to_sample_activations_npz': str(dashboard_sample_file.resolve()),
        'path_to_sample_metadata_pkl': str(sample_metadata_output_file.resolve())
    }
    
    dashboard_index_file = dashboard_cache_output_dir / 'dashboard_cache_index.json'
    try:
...
def consolidate_activations_efficient_command(args: argparse.Namespace):
    ...
    logger.info(f"Executing consolidate_activations_efficient_command with command: {args.command}")

    if args.command == 'consolidate':
        if not hasattr(args, 'cache_dir') or not hasattr(args, 'output_hdf5'):
            logger.error("For 'consolidate' command, --cache-dir and --output-hdf5 are required.")
            return
        ...
        if not all(hasattr(args, attr) for attr in ['consolidated_activations_h5', 'consolidated_metadata_pkl', 'neuron_summaries_dir', 'dashboard_cache_out_dir']):
            logger.error("Missing one or more required arguments for 'create-dashboard-cache' command.")
            return
        
        # Use defaults for optional args if not provided
        n_neurons_sample = args.n_neurons_sample if hasattr(args, 'n_neurons_sample') and args.n_neurons_sample is not None else 100
        n_examples_sample = args.n_examples_sample if hasattr(args, 'n_examples_sample') and args.n_examples_sample is not None else 50000
    ...
    # and should not be called from here to avoid circular dependencies or misplaced logic.
        ...
        logger.error(f"Unknown command for consolidate_activations_efficient_command: {args.command}")
