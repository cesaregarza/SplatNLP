...
def populate_feature_binned_samples(conn: sqlite3.Connection, csv_path: Path):
    ...
    try:
        ...
        if not all(col in df.columns for col in expected_columns):
            logger.error(
                f"CSV file {csv_path} is missing one or more expected columns: {expected_columns}. Found: {list(df.columns)}"
            )
        ...
        logger.info(
            f"Successfully populated feature_binned_samples with {len(df)} rows."
        )
        ...
        logger.error(
            f"Error populating feature_binned_samples: {e}", exc_info=True
        )
        raise
...
def populate_feature_stats_new(conn: sqlite3.Connection, csv_path: Path):
    ...
    try:
        ...
        if df_samples.empty:
            logger.warning(
                "Binned samples CSV is empty. No data to populate feature_stats_new."
            )
            return
        ...
        for feature_id, feature_df in grouped_by_feature:
            ...
            # Estimated Mean
            feature_df["bin_midpoint"] = (
                feature_df["bin_min_activation"]
                + feature_df["bin_max_activation"]
            ) / 2
            # Count samples per original bin_index for weighting
            samples_per_bin = feature_df.groupby("bin_index")[
                "example_id"
            ].count()
            ...
            unique_bins_for_feature = (
                feature_df[["bin_index", "bin_midpoint"]]
                .drop_duplicates()
                .set_index("bin_index")
            )
            ...
            hist_counts = (
                feature_df.groupby("bin_index")
                .size()
                .reindex(
                    np.arange(
                        feature_df["bin_index"].min(),
                        feature_df["bin_index"].max() + 1,
                    ),
                    fill_value=0,
                )
            )
            ...
            bin_ranges_df = (
                feature_df[
                    ["bin_index", "bin_min_activation", "bin_max_activation"]
                ]
                .drop_duplicates()
            ...
            full_bin_indices = pd.Series(
                range(min_bin_idx, max_bin_idx + 1), name="bin_index"
            )

            # Merge to get all bin ranges, then fill missing counts with 0
            # This assumes bin_ranges_df has unique bin_index entries
            temp_hist_df = pd.merge(
                full_bin_indices, bin_ranges_df, on="bin_index", how="left"
            )
            ...
            actual_bin_indices_with_samples = sorted(
                feature_df["bin_index"].unique()
            )
            final_bin_ranges = []
            final_counts = []
            ...
            bin_map = {
                row.bin_index: [row.bin_min_activation, row.bin_max_activation]
                for _, row in current_feature_bins.iterrows()
            }
            ...
            hist_json_counts = [
                counts_per_bin[b_idx] for b_idx in hist_bin_indices
            ]
        ...
        if stats_data:
            ...
            conn.commit()
            logger.info(
                f"Successfully populated feature_stats_new for {len(stats_data)} features."
            )
        else:
        ...
        FileNotFoundError
    ):  # Should be caught by populate_feature_binned_samples if CSV is central
        logger.error(f"Binned samples CSV file not found: {csv_path}")
        raise
    except Exception as e:
...
def populate_top_examples(
    conn: sqlite3.Connection, csv_path: Path, top_n: int = 20
):
    """Populates the top_examples table."""
    logger.info(
        f"Populating top_examples (top {top_n}) using data from {csv_path}..."
    )
    try:
        df_samples = pd.read_csv(csv_path)
        if df_samples.empty:
            logger.warning(
                "Binned samples CSV is empty. No data to populate top_examples."
            )
            return
        ...
        top_n_df = (
            df_sorted.groupby("feature_id").head(top_n).copy()
        ...
        top_examples_to_insert = top_n_df[
        ...
        conn.commit()
        logger.info(
            f"Successfully populated top_examples with {len(top_examples_to_insert)} rows."
        )
    except FileNotFoundError:
        logger.error(f"Binned samples CSV file not found: {csv_path}")
        raise
...
def populate_feature_correlations(
    conn: sqlite3.Connection, json_path: Optional[Path]
):
    """Populates the feature_correlations table from a JSON file."""
    if not json_path:
        logger.info(
            "Correlations JSON path not provided, skipping population of feature_correlations."
        )
        return
    if not json_path.exists():
        logger.warning(
            f"Correlations JSON file not found at {json_path}, skipping."
        )
        return
    ...
    try:
        ...
        if (
            not correlations_data_wrapper
            or "correlations" not in correlations_data_wrapper
        ):
            logger.warning(
                f"Correlations data at {json_path} is missing 'correlations' key or is empty."
            )
            return
        ...
        if not isinstance(correlations_list, list):
            logger.warning(
                f"Correlations data in {json_path} is not a list as expected."
            )
            return
        ...
        for corr_item in correlations_list:
            if all(
                k in corr_item for k in ["neuron_i", "neuron_j", "correlation"]
            ):
        ...
        if db_corr_data:
            ...
            conn.commit()
            logger.info(
                f"Successfully populated feature_correlations with {len(db_corr_data)} items."
            )
        else:
        ...
        logger.error(
            f"Error populating feature_correlations: {e}", exc_info=True
        )
...
def populate_logit_influences(conn: sqlite3.Connection, jsonl_path: Path):
    ...
    if not jsonl_path.exists():
        logger.warning(
            f"Logit influences file not found at {jsonl_path}, skipping population."
        )
        return
    ...
    try:
        with open(jsonl_path, "r") as f:
            for line_num, line in enumerate(f, 1):
                try:
                    ...
                    logger.error(
                        f"Skipping malformed JSON line {line_num} in {jsonl_path}: {line.strip()}"
                    )
                except KeyError as e:
                    logger.error(
                        f"Skipping record due to missing key {e} in line {line_num} in {jsonl_path}"
                    )
        ...
        if logit_influence_records:
            ...
            conn.commit()
            logger.info(
                f"Successfully populated logit_influences with {len(logit_influence_records)} records."
            )
        else:
...
def finalize_feature_stats_table(conn: sqlite3.Connection):
    ...
    try:
        ...
        logger.error(
            f"Error finalizing feature_stats table: {e}", exc_info=True
        )
        raise
...
def ingest_binned_samples_command(args: argparse.Namespace):
    ...
    correlations_json_path = (
        Path(args.correlations_json) if args.correlations_json else None
    )
    logit_influences_path = (
        Path(args.logit_influences_path) if args.logit_influences_path else None
    )
    ...
    try:
        with sqlite3.connect(db_path) as conn:
            ...
            populate_top_examples(
                conn, binned_samples_csv, top_n=args.top_n_examples
            )  # Add top_n_examples to args
            ...
            if correlations_json_path and correlations_json_path.exists():
                ...
                logger.warning(
                    f"Correlations JSON path {args.correlations_json} provided but file not found. Skipping."
                )
            ...
            if logit_influences_path and logit_influences_path.exists():
                ...
                args.logit_influences_path
            ):  # Path provided but file doesn't exist
                logger.warning(
                    f"Logit influences path {args.logit_influences_path} provided but file not found. Skipping."
                )
...
def register_ingest_binned_samples_parser(
    subparsers: argparse._SubParsersAction,
):
    p = subparsers.add_parser(
        "ingest-binned-samples",
        help="Ingest binned samples from CSV and compute feature statistics for the database.",
    )
    p.add_argument(
        "--db-path",
        type=str,
    ...
    p.add_argument(
        ...
        type=str,
        required=True,
        help="Path to the CSV file containing binned samples data (output of extract-top-examples).",
    )
    p.add_argument(
        "--correlations-json",
        type=str,
        default=None,
        help="Optional. Path to JSON file with feature correlations.",
    )
    p.add_argument(
        "--logit-influences-path",
        type=str,
        default=None,
        help="Optional. Path to JSONL file with logit influences.",
    )
    p.add_argument(
        "--top-n-examples",
        type=int,
...
if __name__ == "__main__":
    ...
    logger.warning(
        "This script is designed to be run as a command through cli.py, not directly."
    )
    pass
