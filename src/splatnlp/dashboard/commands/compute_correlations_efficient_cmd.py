    ...
    for neuron_id in tqdm(range(n_neurons), desc="Indexing neurons"):
        ...
        try:
            with open(neuron_file, "rb") as f:
                ...
                if not content:
                    logger.debug(
                        f"Neuron file {neuron_file} is empty, skipping."
                    )
                    continue
                data = orjson.loads(content)  # Use orjson for speed

            current_neuron_examples = set()
            if "range_examples" in data and isinstance(
                data["range_examples"], dict
            ):
                for bin_data in data["range_examples"].values():
                    if "examples" in bin_data and isinstance(
                        bin_data["examples"], list
                    ):
                        for ex in bin_data["examples"][
                            :top_n
                        ]:  # Apply top_n limit here
                            if isinstance(ex, dict) and "index" in ex:
                                ...
                                logger.warning(
                                    f"Malformed example entry in {neuron_file}: {ex}"
                                )
                    else:
                        logger.warning(
                            f"Missing or malformed 'examples' list in bin_data in {neuron_file}"
                        )
            else:
                logger.warning(
                    f"Missing or malformed 'range_examples' in {neuron_file}"
                )
            ...
            logger.warning(
                f"Warning: JSON decode error reading neuron {neuron_id} from {neuron_file}: {e}"
            )
        except Exception as e:
            logger.warning(
                f"Warning: Generic error reading neuron {neuron_id} from {neuron_file}: {e}"
            )

    logger.info(
        f"Built inverted index. example_to_neurons keys: {len(example_to_neurons)}, neuron_to_examples keys: {len(neuron_to_examples)}"
    )
...
def find_candidate_pairs(
    ...
    neuron_ids = sorted(
        neuron_to_examples.keys()
    )  # Neurons that actually have example data
    ...
    if example_to_neurons is None:
        logger.warning(
            "example_to_neurons not provided to find_candidate_pairs, reconstructing. This might be slow."
        )
        example_to_neurons = defaultdict(set)
    ...
    for i, neuron_i in enumerate(tqdm(neuron_ids, desc="Finding pairs")):
        examples_i = neuron_to_examples.get(
            neuron_i, set()
        )  # Use .get for safety
        if (
            len(examples_i) < min_overlap
        ):  # Skip if neuron_i itself has too few examples
            continue
        ...
        for example_id in examples_i:
            ...
            for neuron_j in example_to_neurons.get(example_id, set()):
                if (
                    neuron_j > neuron_i
                ):  # Ensure neuron_j is in our sorted list and after neuron_i
        ...
        valid_overlaps = [
            (neuron_j_id, count)
            for neuron_j_id, count in overlap_counts.items()
            if count >= min_overlap
            and neuron_j_id in neuron_to_examples  # Ensure neuron_j_id is valid
        ]
    ...
    logger.info(
        f"Found {len(candidate_list)} candidate pairs (out of {n_neurons_with_data * (n_neurons_with_data - 1) // 2} possible based on neurons with data)"
    )
...
def fast_pearson_correlation(x: np.ndarray, y: np.ndarray) -> float:
    ...
    if (
        n < 2
    ):  # Pearson needs at least 2 points; standard is 3 for reliability but 2 is min
        return 0.0
    ...
    if (
        den_x_sq <= 1e-9 or den_y_sq <= 1e-9
...
def compute_correlations_for_pairs(
    ...
    logger.info(
        f"Computing {correlation_type} correlations for {len(candidate_pairs)} pairs..."
    )
    ...
    involved_neurons = sorted(
        list(set(n_id for pair in candidate_pairs for n_id in pair))
    )
    if not involved_neurons:
        return []

    # Map these neuron IDs to continuous indices for array slicing
    neuron_id_to_slice_idx = {
        n_id: i for i, n_id in enumerate(involved_neurons)
    }
    ...
    with h5py.File(activations_file, "r") as hf:
        ...
        for neuron_i, neuron_j in tqdm(
            candidate_pairs, desc="Computing correlations"
        ):
            examples_i = neuron_to_examples.get(neuron_i, set())
            examples_j = neuron_to_examples.get(neuron_j, set())
            common_examples_ids = sorted(
                list(examples_i.intersection(examples_j))
            )
            ...
            valid_common_examples_ids = [
                ex_id
                for ex_id in common_examples_ids
                if ex_id < num_total_examples_in_h5
            ]
            ...
            try:
                ...
                if (
                    neuron_i >= num_total_neurons_in_h5
                    or neuron_j >= num_total_neurons_in_h5
                ):
                    logger.warning(
                        f"Neuron index out of bounds for H5 ({neuron_i} or {neuron_j} vs {num_total_neurons_in_h5}). Skipping pair."
                    )
                    continue

                acts_i_values = acts_dataset[
                    valid_common_examples_ids, neuron_i
                ]
                acts_j_values = acts_dataset[
                    valid_common_examples_ids, neuron_j
                ]
            except IndexError as e:
                logger.warning(
                    f"IndexError loading activations for pair ({neuron_i}, {neuron_j}) with examples {valid_common_examples_ids}: {e}"
                )
                continue  # Skip this pair

            if (
                acts_i_values.shape[0] < min_common_for_corr
            ...
            try:
                ...
                if correlation_type == "pearson":
                    corr_val = fast_pearson_correlation(
                        acts_i_values, acts_j_values
                    )
                elif correlation_type == "spearman":
                    # spearmanr can fail with "Too few distinct values" if inputs are constant-like
                    if (
                        len(np.unique(acts_i_values)) < 2
                        or len(np.unique(acts_j_values)) < 2
                    ):
                ...
                logger.error(
                    f"Error computing correlation for pair ({neuron_i}, {neuron_j}): {e}",
                    exc_info=False,
                )  # Keep log clean
...
def compute_correlations_efficient_command(args: argparse.Namespace):
    ...
    try:
        with h5py.File(activations_file_path, "r") as hf:
            if "activations" not in hf:
                logger.error(
                    f"'activations' dataset not found in {activations_file_path}"
                )
                return
            _, total_neurons_in_file = hf["activations"].shape
    except Exception as e:
        logger.error(
            f"Could not read HDF5 file {activations_file_path} to determine neuron count: {e}"
        )
        return
    ...
    logger.info(
        f"Processing up to {n_neurons_to_process} neurons from a total of {total_neurons_in_file} in the HDF5 file."
    )
    ...
    if not neuron_to_examples_map:
        logger.warning(
            "Neuron to examples map is empty. No correlations can be computed. Check neuron data files."
        )
        ...
        stats_data = {
            "total_possible_pairs": n_neurons_to_process
            * (n_neurons_to_process - 1)
            // 2,
        ...
        candidate_neuron_pairs = find_candidate_pairs(
            neuron_to_examples_map,
            args.min_overlap,
            args.max_pairs_per_neuron,
            example_to_neurons_map,
        )
        ...
        significant_correlations_list = [
            corr_item
            for corr_item in computed_correlations
            if abs(corr_item["correlation"])
            >= args.threshold  # Use >= for threshold
        ]
        ...
        # Stats for output
        num_actual_neurons_in_candidates = len(
            set(n for p in candidate_neuron_pairs for n in p)
        )
        stats_data = {
            ...
            "total_possible_pairs_among_processed": n_neurons_to_process
            * (n_neurons_to_process - 1)
            // 2,
            ...
            "significant_correlations_found": len(
                significant_correlations_list
            ),
        }
    ...
    with open(
        output_file_path, "w"
    ) as f:  # Use 'w' for text, standard json.dump
    ...
    if correlation_data_final["correlations"]:  # Only if there are correlations
        ...
        try:
            ...
            logger.info(
                f"Saved sparse correlation matrix to {sparse_matrix_file_path}"
            )
        except Exception as e:
            logger.error(f"Failed to save sparse matrix: {e}")
    else:
        logger.info(
            "No significant correlations to save in sparse matrix format."
        )
    ...
    logger.info(
        f"Total computation time: {time.time() - start_time_total:.2f} seconds"
    )
