...
from typing import Dict, Iterator, List, Optional, Tuple # Ensure all necessary types are imported
...
try:
    ...
    logger.warning("splatnlp.dashboard.database_manager.DashboardDatabase not found. Command may fail if database operations are called.")
    # Define a dummy class if not found, so the rest of the script can be parsed.
    class DashboardDatabase:
        def __init__(self, db_path):
            logger.warning(f"Using dummy DashboardDatabase for {db_path}. Operations will likely fail.")
            self.db_path = db_path
        def get_connection(self): raise NotImplementedError("Dummy DB")
        def insert_examples_batch(self, examples_batch): raise NotImplementedError("Dummy DB")
        def insert_activations_batch(self, activations_array, example_ids_batch, activation_threshold): raise NotImplementedError("Dummy DB")
        def get_database_info(self): raise NotImplementedError("Dummy DB")
        def compute_and_store_feature_stats(self, feature_id): raise NotImplementedError("Dummy DB")
        def get_feature_activations(self, feature_id): raise NotImplementedError("Dummy DB")
        def store_top_examples(self, feature_id, example_ids, activations, top_k): raise NotImplementedError("Dummy DB")
    ...
    if chunk_file.suffix == '.npz':
        ...
        try:
            with np.load(chunk_file) as data:
                ...
                if 'hidden_activations' not in data:
                    logger.warning(f"'hidden_activations' key not found in NPZ file: {chunk_file}. Skipping.")
                    return
                ...
                for i in range(n_examples_in_chunk):
                    ...
                    metadata = {
                        'weapon_id': data.get('weapon_ids', np.array([-1]*n_examples_in_chunk))[i] if 'weapon_ids' in data else -1,
                        'weapon_name': data.get('weapon_names', ['Unknown']*n_examples_in_chunk)[i] if 'weapon_names' in data else 'Unknown',
                        'ability_input_tokens': data.get('ability_input_tokens_list', [[]]*n_examples_in_chunk)[i] if 'ability_input_tokens_list' in data else [],
                        # Add other fields if they exist in your NPZ structure
                        'is_null_token': bool(data.get('is_null_token_flags', [False]*n_examples_in_chunk)[i]) if 'is_null_token_flags' in data else False,
        ...
        try:
            with open(chunk_file, 'rb') as f:
                chunk_data = pickle.load(f) # Expects dict with 'activations' and 'metadata' (DataFrame)
            
            if not isinstance(chunk_data, dict) or 'activations' not in chunk_data or 'metadata' not in chunk_data:
                logger.warning(f"Invalid PKL chunk format in {chunk_file}. Expected dict with 'activations' and 'metadata'. Skipping.")
                return

            activations = chunk_data['activations'] # Should be np.ndarray
            metadata_df = chunk_data['metadata']    # Should be pd.DataFrame

            if not isinstance(activations, np.ndarray) or not isinstance(metadata_df, pd.DataFrame):
                logger.warning(f"Invalid data types for activations or metadata in PKL chunk {chunk_file}. Skipping.")
                return
            if activations.shape[0] != len(metadata_df):
                logger.warning(f"Mismatch in record count between activations ({activations.shape[0]}) and metadata ({len(metadata_df)}) in {chunk_file}. Skipping.")
                return

            for i in range(len(activations)):
                metadata = metadata_df.iloc[i].to_dict()
                metadata['chunk_source_file'] = chunk_file.name # For tracking origin
                yield i, metadata, activations[i]
        except Exception as e:
            logger.error(f"Error reading PKL chunk {chunk_file}: {e}")
            return # Stop iteration for this chunk
    else:
        logger.warning(f"Unsupported chunk file type: {chunk_file.suffix} for file {chunk_file}. Skipping.")
        return
...
def process_single_chunk_streaming(
    ...
    logger.info(f"Streaming data from chunk: {chunk_file.name}, starting global example ID: {current_global_example_id}")
    
    for _, metadata_dict, activation_vector_data in stream_chunk_data(chunk_file):
        ...
        example_data_for_db = {
            ...
            'ability_input_tokens': metadata_dict.get('ability_input_tokens', []), # Ensure this is a list
            'input_abilities_str': metadata_dict.get('input_abilities_str', ''),   # Ensure this is a string
            'top_predicted_abilities_str': metadata_dict.get('top_predicted_abilities_str', ''), # Ensure this is a string
            ...
            'metadata': {k: v for k, v in metadata_dict.items() if k not in [
        ...
        activations_batch_to_insert.append(activation_vector_data) # Append the numpy array directly
        ...
        if len(examples_batch_to_insert) >= batch_size:
            ...
            activations_array_for_db = np.stack(activations_batch_to_insert, axis=0)
    ...
    logger.info(f"Finished processing chunk {chunk_file.name}. Processed {examples_processed_in_this_chunk} examples.")
...
def _streaming_consolidate_main_logic(
    ...
    chunk_files = sorted(list(cache_dir.glob("*.pkl")) + list(cache_dir.glob("*.npz")))
    
    if not chunk_files:
        logger.error(f"No chunk files (.pkl or .npz) found in cache directory: {cache_dir}")
        raise ValueError(f"No chunk files found in {cache_dir}")
    
    logger.info(f"Found {len(chunk_files)} chunk files for potential processing.")
    ...
    if resume and db_path.exists(): # Check if DB exists for resuming
        try:
            ...
            logger.info(f"Resuming. Starting next global example ID at {current_global_example_id_start}. Found {len(processed_chunk_filenames)} already processed chunk filenames in DB.")
        except Exception as e: # Catch broad exceptions for DB issues during resume setup
            logger.error(f"Error during resume setup (accessing DB {db_path}): {e}. Will attempt to run without resuming.")
            current_global_example_id_start = 0 # Reset if resume fails
            processed_chunk_filenames.clear()

    chunks_to_process_paths = [
        cf_path for cf_path in chunk_files if cf_path.name not in processed_chunk_filenames
    ]
    
    if not chunks_to_process_paths:
        logger.info("No new chunk files to process based on resume data.")
    else:
        logger.info(f"Processing {len(chunks_to_process_paths)} new chunk files.")

    total_examples_processed_this_run = 0
    for chunk_file_path_iter in tqdm(chunks_to_process_paths, desc="Streaming chunks to database"):
        try:
            ...
            current_global_example_id_start += num_examples_from_chunk # Update for next chunk
            total_examples_processed_this_run += num_examples_from_chunk
        except Exception as e:
            logger.error(f"Failed to process chunk {chunk_file_path_iter}: {e}", exc_info=True)
            # Decide if to continue with next chunk or stop. For now, continue.
    
    logger.info(f"Streaming consolidation run finished. Processed {total_examples_processed_this_run} examples in this run.")
...
def _compute_analytics_streaming_main_logic(
    ...
    try:
        ...
        logger.error(f"Failed to retrieve feature IDs from database {db_path}: {e}")
        return

    if not all_feature_ids_from_db:
        logger.info("No feature IDs found in the database. Analytics computation cannot proceed.")
        return

    if max_features is not None and max_features > 0: # Apply max_features limit if specified
    ...
    logger.info(f"Computing streaming analytics for {num_features_for_analytics} features, in batches of {features_per_batch}.")
    
    for i in tqdm(range(0, num_features_for_analytics, features_per_batch), desc="Processing feature batches for analytics"):
        current_batch_feature_ids = all_feature_ids_to_process[i : i + features_per_batch]
        ...
        for feature_id_val in current_batch_feature_ids:
            try:
                ...
                activations_for_feature, example_ids_for_feature = db.get_feature_activations(feature_id_val)
                
                if activations_for_feature is not None and len(activations_for_feature) > 0:
                    ...
                    logger.debug(f"No activations retrieved for feature {feature_id_val}, skipping top examples.")
            except Exception as e:
                logger.error(f"Error computing analytics for feature {feature_id_val}: {e}", exc_info=True)
...
def streaming_consolidate_command(args: argparse.Namespace):
    ...
    logger.info(f"Executing streaming_consolidate_command with command: {args.command}")

    if args.command == 'stream-to-db':
        if not all(hasattr(args, attr) for attr in ['cache_dir', 'output_db_path']):
            logger.error("For 'stream-to-db', --cache-dir and --output-db-path are required.")
            return
        # Use defaults for optional args if not provided by CLI
        activation_threshold = args.activation_threshold if hasattr(args, 'activation_threshold') else 1e-6
        batch_size = args.batch_size if hasattr(args, 'batch_size') else 1000
        resume = args.resume if hasattr(args, 'resume') else True # Default to resume
        ...
        if not hasattr(args, 'db_path'):
            ...
            return
        features_per_batch = args.features_per_batch if hasattr(args, 'features_per_batch') else 100
        ...
        logger.error(f"Unknown command for streaming_consolidate_command: {args.command}")
