...
def compute_logit_influences(
    ...
    if feature_id >= sae_model.decoder.weight.shape[0]:
        logger.warning(
            f"Feature ID {feature_id} out of bounds for SAE decoder. Skipping logit influences."
        )
    ...
    with torch.no_grad():
        logit_influences_tensor = torch.matmul(
            output_layer_weight, decoder_vector
        )

    actual_top_k = min(top_k, len(vocab_list))

    # Positive influences
    top_pos_values, top_pos_indices = torch.topk(
        logit_influences_tensor, actual_top_k
    )
    positive_influences = []
    for rank, (val, idx_tensor) in enumerate(
        zip(top_pos_values, top_pos_indices), 1
    ):
        token_idx = idx_tensor.item()
        token_name = (
            vocab_list[token_idx]
            if token_idx < len(vocab_list)
    ...
    # Negative influences
    top_neg_values, top_neg_indices = torch.topk(
        -logit_influences_tensor, actual_top_k
    )
    negative_influences = []
    for rank, (val, idx_tensor) in enumerate(
        zip(top_neg_values, top_neg_indices), 1
    ):
        token_idx = idx_tensor.item()
        token_name = (
            vocab_list[token_idx]
...
def precompute_analytics_command(args: argparse.Namespace):
    ...
    try:
        ...
        for token, index in vocab_map.items():
            if 0 <= index < len(vocab_list):
                ...
                logger.warning(
                    f"Token index {index} for token '{token}' out of bounds for vocab_list size {len(vocab_list)}. Adjusting list size or ignoring token."
                )
                ...
                if index >= len(
                    vocab_list
                ):  # Dynamically extend if necessary, though not ideal
                    vocab_list.extend([""] * (index - len(vocab_list) + 1))
                    vocab_list[index] = token

        inv_vocab_map = {
            name: idx for idx, name in enumerate(vocab_list)
        }  # Used if needed, but vocab_list is primary for index->name
        vocab_size = len(vocab_list)
        logger.info(f"Loaded vocab from {args.vocab_path}, size: {vocab_size}")
        if any(not token_name for token_name in vocab_list):
            logger.warning(
                "Some token indices in vocab_list are empty strings. This might indicate issues with vocab construction if those indices are used."
            )

    except Exception as e:
        logger.error(
            f"Failed to load vocab from {args.vocab_path}: {e}. Cannot proceed.",
            exc_info=True,
        )
        return
    ...
    if hasattr(args, "weapon_vocab_path") and args.weapon_vocab_path:
        try:
            ...
            logger.info(
                f"Loaded weapon vocab from {args.weapon_vocab_path}, size: {weapon_vocab_size}"
            )
        except Exception as e:
            logger.warning(
                f"Could not load weapon_vocab_path: {e}. Using default size {weapon_vocab_size}."
            )
    ...
    try:
        primary_model.load_state_dict(
            torch.load(args.primary_model_checkpoint, map_location=device)
        )
        sae_model.load_state_dict(
            torch.load(args.sae_model_checkpoint, map_location=device)
        )
    ...
    with open(output_file, "w") as f_out:
        for feature_idx in tqdm(
            range(num_sae_features), desc="Computing logit influences"
        ):
            influences = compute_logit_influences(
                ...
                feature_idx,
                vocab_list,
                inv_vocab_map,  # Pass inv_vocab_map too
