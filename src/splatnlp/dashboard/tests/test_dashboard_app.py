...
# Application imports
from splatnlp.dashboard.app import (
    DASHBOARD_CONTEXT as app_context_ref,  # Global context
)
...
from splatnlp.dashboard.components.activation_hist import (
    update_activation_histogram,
)
from splatnlp.dashboard.components.correlations_component import (
    update_correlations_display,
)
from splatnlp.dashboard.components.feature_labels import (  # For context
...
from splatnlp.dashboard.components.top_examples_component import (
    update_top_examples_grid,
)
from splatnlp.dashboard.database_manager import (
    DashboardDatabase,
    DatabaseBackedContext,
)
from splatnlp.preprocessing.transform.mappings import (  # For id_to_name in intervals_grid
    generate_maps,
)
...
MOCK_INV_VOCAB = {
    str(v): k for k, v in MOCK_VOCAB.items()
}  # Ensure string keys for inv_vocab
...
class TestDashboardAppDataSourceDB(unittest.TestCase):
    ...
    def setUp(self):
        ...
        app_context_ref.primary_model = (
            None  # Not testing dynamic tooltips here
        )
    ...
    def _populate_sample_data(self):
        ...
        with self.db_manager.get_connection() as conn:
            ...
            hist2 = json.dumps(
                {"counts": [1, 0, 1], "bin_edges": [0.0, 0.3, 0.6, 0.9]}
            )
            ...
            sample_top_examples_data = [
                (0, 0, 1, 0.95),
                (0, 2, 2, 0.60),
                (0, 1, 3, 0.05),  # Feature 0
                (1, 1, 1, 0.88),
                (1, 3, 2, 0.30),
                (1, 0, 3, 0.10),  # Feature 1
                (2, 2, 1, 0.75),
                (2, 1, 2, 0.50),
                (2, 3, 3, 0.02),  # Feature 2
            ]
            conn.executemany(
                "INSERT INTO top_examples VALUES (?, ?, ?, ?)",
                sample_top_examples_data,
            )
            ...
            conn.executemany(
                "INSERT INTO feature_correlations VALUES (?, ?, ?)",
                sample_feature_correlations_data,
            )
            ...
            sample_logit_influences_data = [
                (0, MOCK_VOCAB["cat"], "cat", 0.9, 1),
                (0, MOCK_VOCAB["mat"], "mat", 0.7, 2),  # Feature 0 Positive
                (
                    0,
                    MOCK_VOCAB["dog"],
                    "dog",
                    -0.8,
                    3,
                ),  # Feature 0 Negative (rank continues)
                (1, MOCK_VOCAB["food"], "food", 0.85, 1),
                (1, MOCK_VOCAB["ate"], "ate", 0.75, 2),  # Feature 1 Positive
                (1, MOCK_VOCAB["sat"], "sat", -0.6, 3),  # Feature 1 Negative
            ]
            conn.executemany(
                "INSERT INTO logit_influences VALUES (?, ?, ?, ?, ?)",
                sample_logit_influences_data,
            )
    ...
    def test_activation_histogram_data_retrieval(self):
        ...
        self.assertTrue(
            len(figure_data_all["data"]) > 0,
            "Histogram data should not be empty for 'all'",
        )
        self.assertEqual(
            figure_data_all["layout"]["title"], "All Activations for Feature 0"
        )
        ...
        self.assertEqual(
            list(figure_data_all["data"][0]["y"]), expected_hist0["counts"]
        )
        ...
        self.assertTrue(
            len(figure_data_nonzero["data"]) > 0,
            "Histogram data should not be empty for 'nonzero'",
        )
        self.assertEqual(
            figure_data_nonzero["layout"]["title"],
            "Non-Zero Activations for Feature 0",
        )
    ...
    def test_correlations_component_data_retrieval(self):
        ...
        sae_corr_display, token_logit_display, error_msg = (
            update_correlations_display(0)
        )

        self.assertEqual(error_msg, "")
        self.assertTrue(
            len(sae_corr_display) > 1,
            "SAE feature correlations display should not be empty.",
        )
        # Expected for feature 0: corr with 1 (0.2), corr with 2 (-0.5)
        # The display is html.P elements. Check their content.
        self.assertIn(
            "Feature 1: 0.200", str(sae_corr_display[1])
        )  # Assuming H5 title is first
        self.assertIn("Feature 2: -0.500", str(sae_corr_display[2]))

        self.assertTrue(
            len(token_logit_display) > 1,
            "Token logit influences display should not be empty.",
        )
        # Expected for feature 0: cat (0.9), mat (0.7), dog (-0.8)
        self.assertIn(
            "Token 'cat': Influence 0.900", str(token_logit_display[1])
        )
        self.assertIn(
            "Token 'mat': Influence 0.700", str(token_logit_display[2])
        )
        self.assertIn(
            "Token 'dog': Influence -0.800", str(token_logit_display[3])
        )
    ...
    def test_top_examples_component_data_retrieval(self):
        ...
        self.assertEqual(
            len(grid_data), 3
        )  # Expecting 3 top examples for feature 1
        ...
        self.assertEqual(
            grid_data[0]["Weapon"], "Splattershot"
        )  # From example_id 1
        self.assertEqual(
            grid_data[0]["Input Abilities"], "dog ate food"
        )  # From example_id 1
    ...
    def test_intervals_grid_component_data_retrieval(self):
        ...
        self.assertEqual(error_msg, "")
        self.assertTrue(
            len(sections) > 0, "Intervals grid sections should not be empty."
        )
        ...
        for section_div in sections:
            if isinstance(section_div, html.Div):
                for child in section_div.children:
                    if isinstance(child, html.Div) and "Interval" in str(
                        child.children
                    ):
                        found_interval_header = True
                    if isinstance(child, dash.html.Div) and hasattr(
                        child, "children"
                    ):  # Looking for dbc.Row containing cards
                        if child.children and isinstance(
                            child.children[0], dash.html.Div
                        ):  # dbc.Col
                            if hasattr(
                                child.children[0], "children"
                            ) and "Card" in str(
                                type(child.children[0].children)
                            ):
                                found_card = True
                                break
                if found_card:
        ...
        self.assertTrue(
            found_interval_header, "Did not find any interval headers."
        )
        self.assertTrue(
