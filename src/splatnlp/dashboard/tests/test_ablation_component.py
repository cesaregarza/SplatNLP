...
class TestAblationComponent(unittest.TestCase):
    def setUp(self):
        ...
        self.mock_primary_model.token_embed = MagicMock(return_value=torch.randn(1, 3, 128)) # batch, seq, d_model
        self.mock_primary_model.weapon_embed = MagicMock(return_value=torch.randn(1, 1, 128)) # batch, 1, d_model
        self.mock_primary_model.input_proj = MagicMock(side_effect=lambda x: x) # Identity
        ...
        self.mock_primary_model.masked_mean = MagicMock(return_value=torch.randn(1, 128)) # batch, d_model
        self.mock_sae_model.encode = MagicMock(return_value=(None, torch.randn(1, 256))) # _, h_post (batch, d_sae)
        ...
        self.mock_context_object.feature_labels_manager.get_display_name = MagicMock(side_effect=lambda x: f"Feature {x}")
    ...
    def test_get_sae_activations_success(self):
        ...
        result = get_sae_activations(
            self.mock_primary_model, self.mock_sae_model, ability_tokens, weapon_token, self.pad_token_id, self.device
        )
    ...
    def test_get_sae_activations_no_primary_model(self):
        result = get_sae_activations(
            None, self.mock_sae_model, [101], 201, self.pad_token_id, self.device
        )
        self.assertIsNone(result)

    def test_get_sae_activations_no_sae_model(self):
        result = get_sae_activations(
            self.mock_primary_model, None, [101], 201, self.pad_token_id, self.device
        )
        self.assertIsNone(result)

    def test_get_sae_activations_model_internal_error(self):
        self.mock_primary_model.masked_mean.side_effect = Exception("Internal model error")
        result = get_sae_activations(
            self.mock_primary_model, self.mock_sae_model, [101], 201, self.pad_token_id, self.device
        )
        self.assertIsNone(result)
    ...
    def test_run_ablation_no_primary_data(self, mock_dashboard_context_obj, mock_get_sae_activations):
        ...
        result = run_ablation_analysis(n_clicks=1, primary_data=None, secondary_build_string="AbilityX", selected_feature_id=0)
    ...
    def test_run_ablation_no_secondary_input(self, mock_dashboard_context_obj, mock_get_sae_activations):
        ...
        result = run_ablation_analysis(n_clicks=1, primary_data=primary_data, secondary_build_string=" ", selected_feature_id=0)
    ...
    def test_run_ablation_get_primary_activations_fails(self, mock_dashboard_context_obj, mock_get_sae_activations):
        ...
        result = run_ablation_analysis(n_clicks=1, primary_data=primary_data, secondary_build_string="AbilityX", selected_feature_id=0)
    ...
    def test_run_ablation_get_secondary_activations_fails(self, mock_dashboard_context_obj, mock_get_sae_activations):
        ...
        primary_data = {'weapon_id_token': 1, 'ability_input_tokens': [100]}
        result = run_ablation_analysis(n_clicks=1, primary_data=primary_data, secondary_build_string="AbilityX,AbilityY", selected_feature_id=0)
    ...
    def test_run_ablation_success_feature_selected(self, mock_dashboard_context_obj, mock_get_sae_activations):
        ...
        mock_dashboard_context_obj.device = self.mock_context_object.device
        mock_dashboard_context_obj.feature_labels_manager = self.mock_context_object.feature_labels_manager
        mock_dashboard_context_obj.feature_labels_manager.get_display_name.return_value = "Awesome Feature 1"
        ...
        mock_get_sae_activations.assert_has_calls([
            ...
            call(self.mock_context_object.primary_model, self.mock_context_object.sae_model, [101, 102], 1, self.mock_context_object.pad_token_id, self.mock_context_object.device)
        ])
        mock_dashboard_context_obj.feature_labels_manager.get_display_name.assert_called_once_with(selected_idx)

        self.assertIsInstance(result, html.Div)
        result_str = str(result)
        self.assertIn("Ablation for Awesome Feature 1:", result_str)
        self.assertIn(f"Primary Activation: {primary_acts[selected_idx]:.4f}", result_str)
        self.assertIn(f"Secondary Activation: {secondary_acts[selected_idx]:.4f}", result_str)
        self.assertIn(f"Difference: {secondary_acts[selected_idx] - primary_acts[selected_idx]:.4f}", result_str)
    ...
    def test_run_ablation_selected_feature_out_of_bounds(self, mock_dashboard_context_obj, mock_get_sae_activations):
        ...
        primary_data = {'weapon_id_token': 1, 'ability_input_tokens': [100]}
        result = run_ablation_analysis(n_clicks=1, primary_data=primary_data, secondary_build_string="AbilityX", selected_feature_id=10)
    ...
    def test_run_ablation_selected_feature_invalid_format(self, mock_dashboard_context_obj, mock_get_sae_activations):
        ...
        primary_data = {'weapon_id_token': 1, 'ability_input_tokens': [100]}
        result = run_ablation_analysis(n_clicks=1, primary_data=primary_data, secondary_build_string="AbilityX", selected_feature_id="not-an-int")

        self.assertIsInstance(result, html.Div)
        self.assertIn("Error: Invalid feature ID format: not-an-int", result.children)
    ...
    def test_run_ablation_feature_labels_manager_missing(self, mock_dashboard_context_obj, mock_get_sae_activations):
        ...
        primary_data = {'weapon_id_token': 1, 'ability_input_tokens': [100]}
        result = run_ablation_analysis(n_clicks=1, primary_data=primary_data, secondary_build_string="AbilityX", selected_feature_id=selected_idx)
    ...
    def test_run_ablation_no_valid_secondary_tokens_after_parse(self, mock_dashboard_context_obj, mock_get_sae_activations):
        ...
        mock_get_sae_activations.assert_called_once_with(
            self.mock_context_object.primary_model, self.mock_context_object.sae_model, [100], 1, self.mock_context_object.pad_token_id, self.mock_context_object.device
        )
