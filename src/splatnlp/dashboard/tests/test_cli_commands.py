...
from splatnlp.dashboard.cli import (
    main as cli_main,  # To test parser and command dispatch
)
...
class TestConsolidateToDbCommand(unittest.TestCase):
    ...
    @mock.patch(
        "splatnlp.dashboard.commands.consolidate_to_db_cmd.DashboardDatabase"
    )
    def test_consolidate_all_inputs_provided(self, MockDashboardDatabase):
            ...
            "features": [
                {
                    ...
                    "top_logit_influences": {
                        "positive": [{"token_name": "A", "influence": 0.9}],
                        "negative": [{"token_name": "B", "influence": -0.8}],
                    },
        ...
        for (
            ...
            mock_db_instance.get_connection.return_value.executemany.call_args_list
        ):
            ...
            if "INSERT OR REPLACE INTO feature_correlations" in sql_query:
                ...
                self.assertEqual(
                    len(call.args[1]), 2
                )  # Two logit influence items (1 pos, 1 neg)
        ...
        self.assertTrue(
            found_correlations_insert, "Feature correlations insert not called."
        )
        self.assertTrue(found_stats_insert, "Feature stats insert not called.")
        self.assertTrue(
            found_logits_insert, "Logit influences insert not called."
        )
        self.assertTrue(
            found_top_examples_insert, "Top examples insert not called."
        )
    ...
    def test_consolidate_missing_analytics_file(self, MockDashboardDatabase):
        ...
        args = argparse.Namespace(
            ...
            input_precomputed_analytics_path=str(
                self.temp_path / "non_existent_analytics.joblib"
            ),
        )
        ...
        for (
            ...
            self.assertNotIn(
                "INSERT OR REPLACE INTO logit_influences", sql_query
            )
...
class TestGenerateActivationsCommandInvocation(unittest.TestCase):
    ...
    @mock.patch(
        "splatnlp.dashboard.commands.generate_activations_cmd.generate_activations_command"
    )
    @mock.patch("argparse.ArgumentParser.parse_args")
    def test_cli_invokes_generate_activations(
        self, mock_parse_args, mock_generate_activations_func
    ):
        ...
        cli_args = [
            ...
            "mock_vocab.json",
            "--weapon-vocab-path",
            "mock_weapon_vocab.json",
            "--data-path",
            "mock_data.csv",
            "--output-path",
            "mock_output_cache.joblib",
        ...
        mock_args_obj = argparse.Namespace(
            ...
            fraction=0.1,
            chunk_size=0.01,
            chunk_storage_dir="activation_chunks_tmp",
            random_state=42,
            embedding_dim=32,
            hidden_dim=512,
            num_layers=3,
