...
class DashboardDatabase:
    ...
    def insert_activations_batch(
        ...
        """
        logger.warning("DEPRECATED: insert_activations_batch is no longer supported for the new schema. Activations are now stored in feature_binned_samples.")
    ...
    def get_binned_feature_samples(self, feature_id: int) -> List[Dict[str, Any]]:
        """
        ...
        if not rows:
            logger.debug(f"No binned samples found for feature_id: {feature_id}")
            return []

        binned_data: Dict[Tuple[int, float, float], List[Dict[str, Any]]] = {}
        for row in rows:
            bin_key = (row['bin_index'], row['bin_min_activation'], row['bin_max_activation'])
        ...
        logger.debug(f"Returning {len(result_list)} bins for feature_id: {feature_id}")
        return result_list
        ...
        with self.get_connection() as conn:
            ...
            for row in cursor.fetchall():
                ...
                if record.get('ability_input_tokens'):
                    try:
                        record['ability_input_tokens'] = json.loads(record['ability_input_tokens'])
                    except json.JSONDecodeError:
                        logger.warning(f"Failed to parse ability_input_tokens for example {record['example_id']}")
                        record['ability_input_tokens'] = [] # Default to empty list
                if record.get('metadata'):
                    try:
                        record['metadata'] = json.loads(record['metadata'])
                    except json.JSONDecodeError:
                        logger.warning(f"Failed to parse metadata for example {record['example_id']}")
        ...
        with self.get_connection() as conn:
            ...
            if histogram_json_str:
                try:
                    ...
                    logger.error(f"Failed to parse sampled_histogram_data for feature {feature_id}")
    ...
    def compute_and_store_feature_stats(self, feature_id: int) -> None:
        ...
        """
        logger.warning("DEPRECATED: compute_and_store_feature_stats is deprecated. Stats are precomputed by ingestion commands.")
        # Optionally, raise an error or simply do nothing.
...
class DatabaseBackedContext:
    ...
    def get_top_examples(self, feature_id: int, limit: int = 20) -> List[Dict[str, Any]]:
    ...
    def get_logit_influences(self, feature_id: int, limit: int = 10) -> Dict[str, List[Dict[str, Any]]]:
        ...
        cached_val = self._cache.get(cache_key, {"positive": [], "negative": []})
        return {
            "positive": [item.copy() for item in cached_val.get("positive", [])],
            "negative": [item.copy() for item in cached_val.get("negative", [])]
        }
