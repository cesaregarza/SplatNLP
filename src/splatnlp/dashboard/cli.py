...
from splatnlp.dashboard.app import app, DASHBOARD_CONTEXT as app_context_ref
...
def load_dashboard_data(args_ns: argparse.Namespace): # Use Namespace type hint
    ...
    logger.info(f"Loaded vocab size: {len(vocab)}, weapon vocab size: {len(weapon_vocab)}")
    ...
    if args_ns.token_activations_path: # Check if path is provided
        if os.path.exists(args_ns.token_activations_path):
            try:
                token_activations_accessor = h5py.File(args_ns.token_activations_path, 'r')
                logger.info(f"Successfully loaded token activations HDF5 file from {args_ns.token_activations_path}")
            except Exception as e:
                logger.error(f"Failed to load token activations HDF5 file from {args_ns.token_activations_path}: {e}")
                token_activations_accessor = None # Ensure it's None on failure
        else:
            logger.warning(f"Token activations path provided ({args_ns.token_activations_path}) but file not found.")
            # token_activations_accessor remains None

    if os.path.exists(args_ns.activations_cache_path):
        logger.info(f"Loading cached activations from {args_ns.activations_cache_path} ...")
        ...
        if isinstance(analysis_df_records_data, list):
            ...
            logger.error("Cached 'analysis_df_records' is not a list of dicts or a DataFrame.")
            raise ValueError("Cached 'analysis_df_records' is not a list of dicts or a DataFrame.")
        logger.info(f"Loaded cached activations: {all_sae_hidden_activations.shape}")
    else:
        logger.error(f"Activations cache not found at {args_ns.activations_cache_path}.")
        logger.info("Please generate the activations cache first using a separate script.")
        raise FileNotFoundError(f"Activations cache not found: {args_ns.activations_cache_path}. Pre-generate this file.")
...
def main():
    ...
    app_context_ref.all_sae_hidden_activations = dashboard_data_obj.all_sae_hidden_activations
    app_context_ref.token_activations_accessor = dashboard_data_obj.token_activations_accessor # Add this line
    app_context_ref.device = dashboard_data_obj.device
    
    logger.info(f"Populated DASHBOARD_CONTEXT in app.py. Type of analysis_df_records: {type(app_context_ref.analysis_df_records)}")
    if app_context_ref.token_activations_accessor:
        logger.info(f"Token activations accessor loaded: {app_context_ref.token_activations_accessor.filename}")
    else:
        logger.info("Token activations accessor not loaded (path not provided or file error).")
